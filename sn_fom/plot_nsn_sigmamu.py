...
class NSN_Syste:
    """
    class to estimate nsn vs z (systematics) for a set of redshift completeness survey
   ...
   nsn_bias_syste: list(str)
    ...
    def __init__(self, nsn_bias, nsn_bias_syste,
        ...
        syste_pos, syste_neg = self.get_syste_fields(
            data, data_syste, fieldNames)

        # save in files

        for key, vals in data.items():
            outName_pos = '{}_plus_1_sigma.hdf5'.format(key)
            outName_neg = '{}_minus_1_sigma.hdf5'.format(key)
            syste_pos[key].to_hdf(outName_pos, key='syste_pos')
            syste_neg[key].to_hdf(outName_neg, key='syste_neg')

        # now make the plot
        fieldName = 'CDFS'
        fig, ax = plt.subplots(figsize=(12, 9))
        for key, vals in data.items():
            ida = vals['fieldName'] == fieldName
            syspos = syste_pos[key]
            sysneg = syste_neg[key]
            idb = syspos['fieldName'] == fieldName
            idc = sysneg['fieldName'] == fieldName

            self.plot_field(ax, fieldName, vals[ida],
                            zcomps, ls, syspos[idb], sysneg[idc])

    def get_syste_fields(self, data, data_syste, fieldNames=['COSMOS', 'XMM-LSS', 'ELAIS', 'CDFS', 'ADFS']):
        """
        Method to estimate systematics 
        ...
        data: dict(df)
          nominal data
        data_syste: dict(dc)
          data with systematic effects included
        fieldNames: list(str),opt
          list of fields to process (default: 
          'COSMOS', 'XMM-LSS', 'ELAIS', 'CDFS', 'ADFS')
        """

        vvars = ['z', 'nsn_eff', 'surveytype', 'zcomp', 'zsurvey', 'fieldName']
        syste_pos = {}
        syste_neg = {}
        ...
        for key, vals in data.items():
            ...
            for field in fieldNames:
                ...
                sel = vals[idx]
                surveytype = sel['surveytype'].unique()[0]
                zsurvey = sel['zsurvey'].unique()[0]
                ...
                syste['surveytype'] = surveytype
                syste['zsurvey'] = zsurvey
                syste['fieldName'] = field

                vvp = pd.DataFrame(syste)
                vvp['nsn_eff'] = vvp['nsn_eff_plus']
                syspos = pd.concat((syspos, vvp[vvars]))

                vvm = pd.DataFrame(syste)
                vvm['nsn_eff'] = vvm['nsn_eff_minus']
                sysneg = pd.concat((sysneg, vvm[vvars]))
            syste_pos[key] = syspos
            syste_neg[key] = sysneg

        return syste_pos, syste_neg

    def plot_field(self, ax, field, sel, zcomps, ls, syste_pos, syste_neg):
        """
        This is where the plot is effectively made
        ...
        for zcomp in zcomps:
            ...
            """
            if not syste_pos.empty:

                idxp = syste_pos['zcomp'] == zcomp
                selsyst_pos = syste_pos[idxp].to_records(index=False)
                idxn = syste_neg['zcomp'] == zcomp
                selsyst_neg = syste_neg[idxn].to_records(index=False)

                nsn_tot = np.sum(selb['nsn_eff'])
                nsn_tot_plus = np.sum(selsyst_pos['nsn_eff'])
                nsn_tot_minus = np.sum(selsyst_neg['nsn_eff'])
                ax.fill_between(
                    selsyst_pos['z'], selsyst_pos['nsn_eff'], selsyst_neg['nsn_eff'], color='yellow')
                """
