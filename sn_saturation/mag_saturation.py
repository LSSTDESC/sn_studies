...
class MagToFlux:
    ...
    def mag_flux(self,mag,band,exptime,nexp):
        ...
        flux_e_sec = self.telescope.mag_to_flux_e_sec(mag,[band]*len(mag),[exptime]*len(mag),[nexp]*len(mag))[:,1]
    ...
    def plot(self,savePlot=False):
        ...
        for band in self.bands:
            ...
            sel = self.data[idx]
            plt.semilogy(sel['mag'],sel['flux_e_sec'],color=self.filtcols[band],label='{} band'.format(band))
...
class MagSaturation:
    ...
    def __init__(self, airmass=1.2, psf_type='single_gauss',bands='gri'):
        ...
        pixel_npy = np.load('PSF_pixels_{}_summary.npy'.format(psf_type))
        pixel_max = interp1d(pixel_npy['seeing'],pixel_npy['pixel_frac_med'],fill_value = 0.0, bounds_error=False)
    ...
    def mag_sat_band(self, band, exptime, full_well):
        ...
        nv = len(self.mag)
        flux_e_sec = self.telescope.mag_to_flux_e_sec(self.mag,[band]*nv,[exptime]*nv,[1]*nv)[:,1]
    ...
    for band in bands:
        ...
        for exptime in exptimes:
            for full_well in full_wells:
                ...
                label = '{} s / {}k pe'.format(int(exptime),int(full_well/1000))
                lstyle = lsstyle[exptime]
                col = colors[full_well]
                ax[pos].plot(sela['seeing'],sela['mag'],linestyle=lstyle,color=col,label=label)
        ...
        if pos == 1:
            ...
            ax[pos].legend(loc='center left', bbox_to_anchor=(1, 0.5),ncol=1,fontsize=fontsize)
