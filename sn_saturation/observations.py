...
class Observations:
    ...
    def plotSeeings(self):
        ...
        for band in bands:
            ...
            iloc = loc[band][0]
            jloc = loc[band][1]
            self.plotSeeing(ax[iloc][jloc], sel, band,
                            seeing, 'b', 0., iloc, jloc)
            self.plotSeeing(ax[iloc, jloc], sel, band,
                            'seeingFwhmGeom', 'r', 0.1, iloc, jloc)

            self.plotSeeing(axb[iloc, jloc], sel, band,
                            seeing, 'b', 0., iloc, jloc, vs_time=True)
            self.plotSeeing(axb[iloc, jloc], sel, band,
                            'seeingFwhmGeom', 'r', 0.1, iloc, jloc, vs_time=True)
            self.seeingProba(sel, band, seeing)
            self.seeingProba(sel, band, 'seeingFwhmGeom')
    ...
    def plotSeeing(self, ax, tab, band, seeing, color, xtrans, ipos, jpos, vs_time=False):
        ...
        if not vs_time:
            ax.hist(tab[val], histtype='step', normed=True, bins=64,
                    range=[0.3, 2.5], linewidth=3, label=seeing)
        else:
        ...
        if ipos == 0 and jpos == 0:
            ax.legend(loc='upper center', bbox_to_anchor=(1.1, 1.2),
    ...
    def seeingProba(self, tab, band, seeing):
        ...
        val = seeing
        print(band, val, 'med=', np.median(
            tab[val]), tab[val].min(), tab[val].max())
        bin_width = 0.05
        min_val = tab[val].min()
        max_val = tab[val].max()
        r = self.getDistrib(tab, val, bin_width, 'seeing', 'weight_seeing')
        fichname = 'seeing_{}_{}.npz'.format(band, seeing)
        np.savez(fichname, seeing=r['seeing'],
                 weight_seeing=r['weight_seeing'])
        """
        rs=[]
    ...
    def getDistrib(self, sel, varname, bin_width, varnamea, varnameb):
        ...
        hista, bin_edgesa = np.histogram(
            sel[varname], bins=num_bins+1, range=[min_val-bin_width/2., max_val+bin_width/2.])
        ...
        for i, val in enumerate(hista):
            ...
            r.append((round(bin_center[i], 3), float(
                hista[i])/float(np.sum(hista))))
    ...
    def dist(self, cadence=3):
        ...
        for band in 'gri':
            ...
            for mjd in np.arange(mjdmin, mjdmax, 365.):
                ...
                seeingFwhmEff_tile = np.tile(
                    sel['seeingFwhmEff'], (len(mjds), 1))
                seeingFwhmGeom_tile = np.tile(
                    sel['seeingFwhmGeom'], (len(mjds), 1))
                ...
                seeingFwhmGeom_ma = np.ma.array(
                    seeingFwhmGeom_tile, mask=~idxb)
                ...
                resb = rf.append_fields(
                    resb, 'seeingFwhmEff', seeingFwhmEffcol)
                resb = rf.append_fields(
                    resb, 'seeingFwhmGeom', seeingFwhmGeomcol)
        ...
        for band in 'gri':
            ...
            for val in ['m5', 'seeingFwhmEff', 'seeingFwhmGeom']:
                interpol = interp1d(
                    tabb['mjd'], tabb[val], fill_value=-1, bounds_error=False)
                if resfib is None:
        ...
        np.save('distrib_{}.npy'.format(cadence), np.copy(resfi))
    ...
    def make_obs(self, restot, nexp, exptime):
        ...
        for band in np.unique(restot['band']):
            ...
            res = rf.append_fields(res, airmassCol, [1.2]*len(res))
            res = rf.append_fields(res, moonPhaseCol, [0.1]*len(res))
            ...
            res = rf.append_fields(res, obsIdCol, obsId)
