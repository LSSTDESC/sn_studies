...
def filteralloc_stat(df):
    ...
    for dbName in df['dbName'].unique():
        ...
        for vv in ['gir', 'yz', 'giryz', 'g', 'giruy', 'r', 'i', 'z', 'y']:
            ...
            selb = sel[idxb]
            if len(selb) > 0:
                xmin = np.round(100.*np.min(selb[what]), 2)
                xmax = np.round(100.*np.max(selb[what]), 2)
                if xmin >= 1.:
                    print(dbName, vv, xmin, xmax)
...
def calc(grp):
    ...
    for vv in ['gir', 'yz', 'giryz', 'g', 'giruy', 'r', 'i', 'z', 'y']:
        dictres['mean_nights_{}'.format(vv)] = [np.mean(
            grp['nights_{}'.format(vv)]/grp['nights_ddf'])]
        dictres['rms_nights_{}'.format(vv)] = [np.std(
            grp['nights_{}'.format(vv)]/grp['nights_ddf'])]
    ...
    if 'descddf' in grp.name[1]:
        main_seq = 'yz'
        factor_seq = 2
    dictres['mean_nights_seq'] = [factor_seq *
                                  dictres['mean_nights_{}'.format(main_seq)][0]]
...
def load(fis):
    ...
    for fi in fis:
        ...
        db = df.groupby(['fieldName', 'dbName']).apply(
            lambda x: calc(x)).reset_index()
...
def plot(dftot, xvar='dbName_spl', yvar='mean_nonights', yerr='std_nonights', legy='Fraction of nights without observation'):
    ...
    ax1.grid()
    ax2.grid()
    plt.legend(bbox_to_anchor=(0.95, 2.2), ncol=6,
...
def plot_new(dftot, xvar='dbName_spl', yvar='mean_nights_seq', legy='Fraction of nights \n with the full sequence'):
    ...
    plt.legend(bbox_to_anchor=(0.95, 1.1), ncol=6,
               fontsize=fontsize, frameon=False)
...
parser.add_option("--fileDir", type="str",
                  default='../../OS_downtime', help="db dir [%default]")
...
plot(df, yvar='mean_nonights', yerr='std_nonights',
     legy='fraction of nights \n without observation')
"""
plot(df, yvar='mean_cadence', yerr='std_cadence',
     legy='cadence \n of observation [days$^{-1}$]')
plot(df, yvar='mean_season_length',
     yerr='std_season_length', legy='season length [days]')

plot_new(df, yvar='mean_nights_seq')

plot_new(df, yvar='mean_nights_giryz', legy='grizy')
