...
def load(dbName, nside,group=['fieldName']):
    ...
    df = pd.DataFrame(np.load('DD_Summary_{}_{}.npy'.format(dbName,nside),allow_pickle=True))
...
vars_toplot = ['nights','season_length','cadence','maxgap','medgap','missing_nights','area']
legend = ['#nights/season/pixel','season length [day]','cadence [day-1]','max gap [day]','median gap [day]','missing nights','area [deg2]']
