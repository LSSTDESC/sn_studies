...
class RestFrameBands:
    ...
    def waves(self):
        ...
        for band in self.bands:
            mean_restframe_wavelength = telescope.mean_wavelength[band] / (
                1. + zvals)
            arr = np.array(mean_restframe_wavelength,
                           dtype=[('lambda_rf', 'f8')])
    ...
    def zcutoff(self):
        ...
        for band in self.bands:
            ...
            interp = interpolate.interp1d(
                selw['lambda_rf'], selw['z'], bounds_error=False, fill_value=(1.5, 0.0))
    ...
    def plot(self):
        ...
        for band in self.bands:
            ...
            axw.plot(selig['lambda_rf'], selig['z'],
                     color=filtercolors[band], label=band)

        axw.plot([self.blue_cutoff]*2, [0., 1.2], color='r', ls='--')
        axw.plot([self.red_cutoff]*2, [0., 1.2], color='r', ls='--')
        axw.set_xlabel(
            r'$\lambda^{LSST\ mean\ band}_{rf}$ [nm]', fontsize=fontsize)
...
class SignalBand:
    ...
    def __init__(self, lcdf):
        ...
        sumflux_band_z = lcdf.groupby(['x1', 'color', 'band', 'z'])[
            'flux_e_sec'].sum().reset_index()

        # print(type(sumflux_band_z))

        sumflux_z = lcdf.groupby(['x1', 'color', 'z'])[
            'flux_e_sec'].sum().reset_index()

        sumflux_z = sumflux_z.rename(columns={"flux_e_sec": "flux_e_sec_tot"})

        sum_merge = sumflux_band_z.merge(
            sumflux_z, left_on=['x1', 'color', 'z'], right_on=['x1', 'color', 'z'])

        # print(sum_merge)
        #sum_merge['band'] = sum_merge['band'].map(lambda x: x.decode()[-1])
        sum_merge['fracfluxband'] = sum_merge['flux_e_sec'] / \
            sum_merge['flux_e_sec_tot']
