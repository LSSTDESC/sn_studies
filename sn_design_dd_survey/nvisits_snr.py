    ...
    for b in bands:
        ...
        for zv in z:
            if zv<=1:
                ...
                ax.plot(selb['nVisits'],selb['SNR'],color=colors[b],label='{} - z={}'.format(b,zv),ls=ls[zv])
...
def plot_z(zref,sel_snr):
    ...
    for band in np.unique(sel_snr['band']):
        ...
        ax.plot(sel_snrb['nVisits'],sel_snrb['SNR'],color=colors[band],label=band)
        ax.plot(sel_snrb['nVisits'],sel_snrb['SNR_photo_bd'],ls='--',color=colors[band])
        axb.plot(sel_snrb['SNR'],sel_snrb['m5'],color=colors[band],label=band)
...
def plot_SNRmax(sel_snr,what='SNR'):
    ...
    selb= sel_snr.groupby(['band','z'])[what].apply(lambda x : x.max()).reset_index()
