...
class AnaMedValues:
    ...
    def __init__(self, fname):
        ...
        # load the data
        medValues = np.load(fname, allow_pickle=True)
        ...
        dfgrp = df.groupby(['fieldname', 'season', 'filter']
                           ).median().reset_index()

        # estimate median per fieldname,filter
        dfgrp_season = df.groupby(
    ...
    def plot(self, dfgrp, dfgrp_season):
        ...
        axres.set_ylabel(
            'median m$_{5}^{single visit}$ [ten seasons]', fontsize=fontsize)
        axres.grid()
...
class Anadf:
    ...
    def __init__(self, datadf):
        ...
        self.dfana = dfsnr.merge(
            dfzlim, left_on=['x1', 'color'], right_on=['x1', 'color'])
