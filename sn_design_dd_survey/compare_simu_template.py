...
class ErrorModel_Study:
    ...
    def plot(self, simus):
        ...
        for b in self.bands:
            ...
            print(lc.columns)
            self.plotBand(lc,whatx='z',whaty='SNR_errormodel')
    ...
    def getLCs(self, band, simus):
        ...
        lctot = Table()
        for val in simus:
            lc = Table.read(self.lcName, path='lc_{}'.format(val['index_hdf5']))
...
def plotLC(tabs,z,colors,sna,snb):
    ...
    deltaph = []
    for b in 'grizy':
        deltaph.append((b,deltaphase(sna,snb,z,b,tabs, whatx='phase',whaty='flux')))
    ...
    for b in 'grizy':
        ...
        for key, tab in tabs.items():
            ...
            """
            ax[i,j].errorbar(sel['phase']+deltapph,sel['flux'],yerr=sel['fluxerr_model'],color=colors[key],marker='o')
    plt.show()
...
def deltaphase(sna,snb,z,b,tabs, whatx='phase',whaty='flux',phases=np.arange(-20.,60.,0.1)):
    ...
    for key,tab in tabs.items():
        ...
        idx &= tab['snr_m5']>=1.
        sel = tab[idx]
        flux_interp= interpolate.interp1d(sel[whatx], sel[whaty], bounds_error=False, fill_value=0.)
...
ErrorModel_Study('Output_Simu_error_model_ebvofMW_0.0', 'Simu_sn_cosmo_Fake_0.2_-0.1_error_model_ebvofMW_0.0_0.hdf5')
